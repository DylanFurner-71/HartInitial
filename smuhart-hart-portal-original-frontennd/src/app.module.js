import LayoutModule from './containers/layout/layout.module.js';
import { StudentService } from './services/student.service.js';
import { IndexController } from './containers/index/index.controller.js';
import { AssessmentController } from './containers/assessment/assessment.controller.js';

const surveyApp = angular.module('surveyApp', [LayoutModule])
  .controller('IndexController', IndexController)
  .controller('ProfileController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.init = function () {
      $scope.error = false;
  ...
  .controller('MyActionplanController', ($scope, $location, $rootScope, $http) => {
    $scope.init = function () {
      $scope.tabIdx = 0;
      $scope.lDirty = 0;
      $http.post('/hclssurvey1/getstudentactionplan.json', null, null).then((response) => {
    ...
    $scope.saveMyActionPlan = function () {
      ...
      $http.post('/hclssurvey1/savestudentactionplan.json', reqObj, null).then((response) => {
  ...
  .controller(
  'mainAppCtrl',
  ($scope, $location, $rootScope, $http, $timeout, user, toastr) => {
    window.scrollTo(0, 0);
    $scope.startNum = 0;
    $scope.endNum = 4;
    ...
    $scope.createRespObj = function (respStr) {
      let respStrArr = respStr.split(',');

      $scope.initializeRespObj();
      let respObj = $scope.responseObj;
      for (i = 0; i < 55; i++) {
        for (j = 0; j < 3; j++) {
          let index = (i * 3) + j;
    ...
    $scope.init = function () {
      ...
      $http.post('/hclssurvey1/getsurveyresult.json', reqObj, null).then((response) => {
  ...
  .controller('AdminController', ($http, $scope, $location, $rootScope, PagerService, filterFilter) => {
    ...
    $scope.init = function () {
       ...
       */
      $http.post('/hclssurvey1/loadstudentbatch.json', null, null).then((response) => {
    ...
    $scope.loadstudents = function (batch) {
      $http.post('/hclssurvey1/getstudentsperbatch.json', batch, null).then((response) => {
    ...
    $scope.initadminhome = function () {
      $http.post('/hclssurvey1/checkadmin.json', null, null).then((response) => {
    ...
    $scope.uploadFile = function (files) {
      ...
      let term = `${$scope.term}-${$scope.termyear}`;
      let studentgroup = $scope.studentgroup;
      let gradstudy = $scope.study;
      fd.append('term', term);
      ...
      $http.post('/hclssurvey1/uploadexcel.json', fd, {
        ...
        { $scope.status = 2; } else if (response.data.status == 1) {
    ...
    $scope.updateDebriefthankyounote = function () {
      ...
      $http.post('/hclssurvey1/updatedebriefdatenote.json', classdate, null, null).then(() => {
        $scope.dateupdated = 1;
    ...
    $scope.getallcurrentvalues = function () {
      ...
      $http.post('/hclssurvey1/getdebriefdatenote.json', inputs, null).then((response) => {
    ...
    $scope.loadbatches = function () {
      ...
      $http.post('/hclssurvey1/getstudentgroups.json', termstudy, null).then((response) => {
    ...
    $scope.getstats = function () {
      ...
      $http.post('/hclssurvey1/getavgscores.json', statinputs, null).then((response) => {
  ...
  .controller('LineGraphController', ($scope, $rootScope, $http, $location, $window, $timeout, toastr, user) => {
    window.scrollTo(0, 0);
    $scope.majorlist = [{
      value: 0,
    ...
    $scope.getFaaWidth = function (i) {
      let lIndex = i * 3;
      let retVal = parseInt($scope.tableElements[lIndex][3]) + parseInt($scope.tableElements[lIndex + 1][3]) + parseInt($scope.tableElements[lIndex + 2][3]);
    ...
    $scope.populateLdrCmpObj = function () {
      let responseObj = $scope.surveyRespObj.surveyResp.split(',');
      for (i = 0; i < 2; i++) {
        $scope.ldrCmpObj[i] = [];
        for (k = 0; k < 3; k++) {
          j = i * 15 + k;
    ...
    $scope.init = function (surveyResponse) {
      ...
      if (surveyResponse != null && surveyResponse.smuId != null) {
        ...
        $http.post('/hclssurvey1/getsurveyresult.json', reqObj, null).then((response) => {
          ...
          let respStrArr = $scope.surveyRespObj.surveyResp.split(',');
        ...
        $http.post('/hclssurvey1/getstudentinfo.json', reqObj, null).then((response) => {
          lFinalObj.info = response.data;
        }, null);
        $http.post('/hclssurvey1/files/quizQues.txt', reqObj, null).then((response) => {
  ...
  .controller('infoCtrl', ($scope, $location, user, $http, toastr, $rootScope) => {
    ...
    $scope.init = function () {
      $http.post('/hclssurvey1/getstudentinfo.json', {
        ...
        if ($scope.information.surveyStatus == 3) {
          ...
          $http.post('/hclssurvey1/getstudentinitialresp.json', null, null).then((response) => {
  ...
  .controller('defaultpageController', ($scope, $http, $location) => {
    $scope.init = function () {
      $http.post('/hclssurvey1/isadmin.json', null, null).then((response) => {
  ...
  .controller('thankYou', ($scope, $location, toastr, $window, $rootScope, user, $http) => {
    window.scrollTo(0, 0);
    ...
    $scope.init = function () {
      ...
      $http.post('/hclssurvey1/getdebriefdatenote.json', batchterm, null).then((response) => {
        let tempthankyounote = response.data.thankyounote;
        $scope.sessiondate = response.data.session_date;
        let debriefdate = new Date(response.data.session_date);
  ...
  .controller('ceController', ($scope, $location, $rootScope, $http, user, toastr) => {
    window.scrollTo(0, 0);
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.cev1 = function () {
      $scope.videoquiz = 1;
  ...
  .controller('coController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.cov1 = function () {
  ...
  .controller('drController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.dr1 = function () {
  ...
  .controller('dlController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.dlv1 = function () {
  ...
  .controller('ddController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.ddv1 = function () {
  ...
  .controller('elController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.elv1 = function () {
  ...
  .controller('eiController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.eiv1 = function () {
  ...
  .controller('isController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.isv1 = function () {
  ...
  .controller('ilController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.ilv1 = function () {
  ...
  .controller('psController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.psv1 = function () {
  ...
  .controller('saController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.saveresp = function (cmp, vno, text) {
      let videoresp = {
        cmp: cmp,
        vno: vno,
        text: text,
      };
      $http.post('/hclssurvey1/savevideoresp.json', videoresp, null).then(null, null);
    };
  })

  .controller('spController', ($scope, $location, $rootScope, $http, user, toastr) => {
    $scope.location = $location;
    $scope.videoquiz = 0;
    console.log($scope.videoquiz);
    $scope.spv1 = function () {
